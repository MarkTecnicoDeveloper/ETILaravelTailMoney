    ...
    <div class="px-6 py-6">
        <div class="bg-white rounded-lg px-4 py-4">
            <div class="flex flex-row h-[45px] items-center">
                <h4 class="text-green-900 font-bold w-4/5">Make extract</h4>
            </div>
            <hr>

            @include('dashboard.includes.alerts')

            <form class="mt-4" method="POST" action="{{ route('withdraw.store') }}">
                @csrf

                <!-- Email Address -->
                <div>
                    <x-input id="email" class="block mt-1 w-full" type="text" name="extractValue" placeholder="Enter the amount to extract" autofocus />
                </div>
